...
public class PointController : MonoBehaviour
{
    ...
    {
        OnError?.Invoke();
        OnErrorInt?.Invoke(_consecutiveErrors);
    }
    ...
    public void CheckConsecutiveEvents(int consecutives)
    {
        ...
        for (int i = 0; i < PointsEventsTrigger.Count; i++)
        {
            ...
            if (consecutives >= evt.PointTrigger && consecutives % evt.PointTrigger == 0)
            {
                evt.EventTrigger?.Invoke();
            }
        }
    }
    public void CheckConsecutiveErrorEvents(int consecutiveErrorsCount)
    {
        if (consecutiveErrorsCount <= 0 || ErrorEventsTrigger == null || ErrorEventsTrigger.Count == 0)
            return;
        ...
        for (int i = 0; i < ErrorEventsTrigger.Count; i++)
        {
            ...
            if (consecutiveErrorsCount >= evt.PointTrigger && consecutiveErrorsCount % evt.PointTrigger == 0)
            {
                evt.EventTrigger?.Invoke();
            }
        }
